...
pcl::HDLGrabber::HDLGrabber(const std::string &correctionsFile,
    ...
    : hdl_data_(), udp_listener_endpoint_(HDL_DEFAULT_NETWORK_ADDRESS, HDL_DATA_PORT), source_address_filter_(), source_port_filter_(443), hdl_read_socket_service_(), hdl_read_socket_(NULL), pcap_file_name_(pcapFile), queue_consumer_thread_(NULL), hdl_read_packet_thread_(NULL), current_scan_xyz_(new pcl::PointCloud<pcl::PointXYZ>()), current_sweep_xyz_(new pcl::PointCloud<pcl::PointXYZ>()), current_scan_xyzi_(new pcl::PointCloud<pcl::PointXYZI>()), current_sweep_xyzi_(new pcl::PointCloud<pcl::PointXYZI>()), current_scan_xyzrgb_(new pcl::PointCloud<pcl::PointXYZRGBA>()), current_sweep_xyzrgb_(new pcl::PointCloud<pcl::PointXYZRGBA>()), current_sweep_src_(new pcl::PointCloud<pcl::PointSrc>), last_azimuth_(65000), sweep_xyz_signal_(), sweep_xyzsrc_signal_(), sweep_xyzrgb_signal_(), sweep_xyzi_signal_(), scan_xyz_signal_(), scan_xyzrgb_signal_(), scan_xyzi_signal_(), min_distance_threshold_(0.0), max_distance_threshold_(10000.0)
{
...
pcl::HDLGrabber::HDLGrabber(const boost::asio::ip::address &ipAddress,
    ...
    : hdl_data_(), udp_listener_endpoint_(ipAddress, port), source_address_filter_(), source_port_filter_(443), hdl_read_socket_service_(), hdl_read_socket_(NULL), pcap_file_name_(), queue_consumer_thread_(NULL), hdl_read_packet_thread_(NULL), current_scan_xyz_(new pcl::PointCloud<pcl::PointXYZ>()), current_sweep_xyz_(new pcl::PointCloud<pcl::PointXYZ>()), current_scan_xyzi_(new pcl::PointCloud<pcl::PointXYZI>()), current_sweep_xyzi_(new pcl::PointCloud<pcl::PointXYZI>()), current_scan_xyzrgb_(new pcl::PointCloud<pcl::PointXYZRGBA>()), current_sweep_xyzrgb_(new pcl::PointCloud<pcl::PointXYZRGBA>()), last_azimuth_(65000), sweep_xyz_signal_(), sweep_xyzrgb_signal_(), sweep_xyzi_signal_(), scan_xyz_signal_(), scan_xyzrgb_signal_(), scan_xyzi_signal_(), min_distance_threshold_(0.0), max_distance_threshold_(10000.0)
{
  initialize(correctionsFile);
}
...
void pcl::HDLGrabber::initialize(const std::string &correctionsFile)
{
  ...
  sweep_xyzsrc_signal_ = createSignal<sig_cb_velodyne_hdl_sweep_point_cloud_xyz_src>();
  ...
  current_sweep_src_.reset(new pcl::PointCloud<pcl::PointSrc>);
...
void pcl::HDLGrabber::processVelodynePackets()
{
  while (true)
  {
    ...
    //std::cout << hdl_data_.size()<< std::endl;
...
void pcl::HDLGrabber::toPointClouds(HDLDataPacket *dataPacket)
{
  ...
  for (int i = 0; i < HDL_FIRING_PER_PKT; ++i)
  {
    ...
    {
      //int newidx = (j % 2 == 0) ? (j / 2) : (j / 2 + 16);
      //if (newidx>=g_selectLineNum)
      //	continue;

      //���if����������Ƕ���С����һ�εĽǶ�֮��Ż�
      //�����źţ�Ҳ���Ǽ���ת��һ�ܣ��Ƕ���359���Ϊ0��
      //�����fireCurrentSweep�������Ƿ����źŵĺ���
      if (firingData.rotationalPosition < last_azimuth_)
      {
        if (current_sweep_xyzrgb_->size() > 0)
        {
          if (current_sweep_xyzrgb_->size() % HDL_LASER_PER_FIRING != 0)
          {
            current_sweep_src_.reset(new pcl::PointCloud<pcl::PointSrc>());
            current_sweep_xyz_.reset(new pcl::PointCloud<pcl::PointXYZ>());
          ...
          current_sweep_src_->is_dense = current_sweep_xyz_->is_dense = current_sweep_xyzrgb_->is_dense = current_sweep_xyzi_->is_dense = false;
          current_sweep_xyz_->header.stamp = velodyneTime;
          current_sweep_src_->header.stamp = velodyneTime;
          ...
          current_sweep_src_->header.seq = sweepCounter;
          for (int k = 0; k < HDL_LASER_PER_FIRING; k++)
          {
            std::vector<HDLLineReturn> swapLine;
            swapLine.swap(current_line_data_[k].HDLLineReturns);
            //current_line_data_[j].HDLLineReturns.clear();
          }
      ...
      PointXYZI xyzi;
      PointXYZRGBA xyzrgb;
      // HDLLineReturn singleReturn;
      //Record radius and angle
      PointSrc ra;

      computeXYZI(xyzi, firingData.rotationalPosition, firingData.laserReturns[j], laser_corrections_[j + offset]);

      //order all firing data into 32 lines

      //if (i == 0)
      //{
      // current_line_data_[j].verticalAngleD = laser_corrections_[j+offset].verticalCorrection;
      //}
      //singleReturn.distance = firingData.laserReturns[j].distance * 0.002;
      //singleReturn.intensity = firingData.laserReturns[j].intensity;
      //singleReturn.rotationalPosition = firingData.rotationalPosition / 100.0;
      //current_line_data_[j].HDLLineReturns.push_back(singleReturn);

      ra.radius = firingData.laserReturns[j].distance;
      ra.angle = firingData.rotationalPosition;
      ...
      current_sweep_src_->push_back(ra);
                                  ...
                                  HDLLaserReturn laserReturn, HDLLaserCorrection correction)
{
  ...
  /*point.x = static_cast<float> (azimuth/1000.0);
  point.y = static_cast<float> (distanceM * correction.cosVertCorrection);
...
void pcl::HDLGrabber::fireCurrentSweep()
{
  ...
  if (sweep_xyzsrc_signal_->num_slots() > 0)
    sweep_xyzsrc_signal_->operator()(current_sweep_xyzi_, current_sweep_src_);
...
/////////////////////////////////////////////////////////////////////////////
//#ifdef HAVE_PCAP
void pcl::HDLGrabber::readPacketsFromPcap()
{
  ...
  while (returnValue >= 0 && !terminate_read_packet_thread_)
  {
    ...
    // The ETHERNET header is 468 bytes long; unnecessary
