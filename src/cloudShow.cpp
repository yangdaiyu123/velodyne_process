...
visualization_msgs::Marker get_box_marker(const Tracker &tracker)
{
    ...
    markerBox.header.frame_id = "velodyne";
...
void cloud_show::show_points(const CTrackersCenter &trackingCenter, const CurbDetection &curbDetection,
                             std::vector<int> &obs_idx, pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud_data)
{
    ...
    *curb_line += curbDetection.side_points_right_;
    ...
    {
        auto marker = get_box_marker(one);
    ...
    {
        pcl::PointXYZI tmp;
        tmp = cloud_data->points[one];
        cloud_obs->push_back(tmp);
    }
    ...
    pcl::toROSMsg(*cloud_data, output4);
