...
ObsCluster::ObsCluster(const pcl::PointCloud<pcl::PointXYZI> &obs_points)
    : _point_ori(obs_points), _pts_cluster(new pcl::PointCloud<pcl::PointXYZI>)
{
    //对障碍物点建立KD树索引
...
void ObsCluster::DBSCAN(float Eps, int MinPts)
{
    ...
    for (int i = 0; i < _point_ori.size(); i++)
    {
        ...
        if (_point_ori[i].x < -10 || _point_ori[i].x > 10)
            continue;
...
float figureSimiCalculation(TrackerObs TrackerObs, GroupProp figure)
{
    float simi = 0.0;
    float geoSimi = 0.0;
    int posSimi = 0;
    ...
    double distance = sqrt(pow(TrackerObs.PropertiesKF._center.x - figure._center.x, 2) + pow(TrackerObs.PropertiesKF._center.y - figure._center.y, 2));
    if (distance > 8.0)
        posSimi = 0;
    else
    ...
    float biggerRadius = max(TrackerObs.PropertiesKF._radius, figure._radius);
    geoSimi = (biggerRadius - abs(TrackerObs.PropertiesKF._radius - figure._radius)) / biggerRadius;
...
void TrackersCenter::inputSingFrameFigures(std::vector<GroupProp> figureList, int frameID, double cur_time)
{
    ...
    if (TrackerList.empty())
    {
        ...
        for (auto &figure : figureList)
        {
            TrackerObs trackerNewOne(getNewObjID());
    ...
    std::vector<int> trackerStatus(numTracker, -1); //fill in code of figure the TrackerObs taking |-1 = missing
    std::vector<int> trackerStatus_new(0);          //fill in code of figure the TrackerObs taking |-1 = missing
    ...
    if (!figureList.empty())
    {
        ...
        //1.2belong
        //find max simi in col(different TrackerObs)
        ...
        //solve multi figure match the same TrackerObs
    ...
    ////2.comb list
    //create new TrackerObs with new figure
    ...
    for (unsigned ifigure = 0; ifigure < figureBelong.size(); ifigure++)
    {
        ...
        {
            TrackerObs newOne(getNewObjID());
            TrackerList.push_back(newOne);
            trackerStatus_new.push_back(ifigure);
        }
    }

    std::cout << "TrackerObs size is " << TrackerList.size() << std::endl;
...
void TrackerObs::create_WithNewMember(const GroupProp &figure)
{
...
void TrackerObs::update_WithNewMember(const GroupProp &figure)
{
...
void TrackerObs::update_WithNoMember()
{
    KF.inputMeasurement(PropertiesKF._center.x, PropertiesKF._center.y);
    KF.outputCorrection(PropertiesKF._center.x, PropertiesKF._center.y);
}

int TrackerObs::getLastFnumber()
{
    return FiguresPro[0].id;
}

int TrackersCenter::getNewObjID()
{
    trackerIDCount++;
